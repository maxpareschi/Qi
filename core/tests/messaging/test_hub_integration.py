...
async def test_register_session_with_real_hub_fixed_v2(
    ...
    with patch(
        "core.messaging.connection_manager.QiConnectionManager.register"
    ) as mock_register:
    ...
    with patch(
        "core.messaging.connection_manager.QiConnectionManager.unregister"
    ) as mock_unregister:
...
async def test_request_response_pattern_with_real_hub_fixed_v2(
    ...
    with patch(
        ...
        with patch("core.messaging.message_bus.QiMessageBus.request") as mock_request:
...
async def test_publish_message_with_real_hub_fixed_v2(
    ...
    with patch(
        ...
        with patch("core.messaging.message_bus.QiMessageBus.publish") as mock_publish:
