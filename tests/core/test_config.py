...
from core.config import CONFIG_FILE, DOTENV_FILE, QiLaunchConfig
...
@patch("core.config.tomllib.load", return_value={})
def test_path_normalization_validators(mock_load, mock_env_vars, mock_config_files):
    ...
    with patch("core.config.tomllib.load", return_value={}):
        # Test custom paths
...
def test_module_qi_launch_config_instance_loads_from_env(mock_exists_module_scope):
    # This test would require reloading the core.config module to be effective.
    ...
    with patch.dict(os.environ, {"QI_HOST": "module_test_host"}):
        ...
        assert core.config.qi_launch_config.host == "module_test_host"
