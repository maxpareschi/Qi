 ...
 * 3. Global variables (set by server/extension)
...
export const qiConnection = $state({
   ...
   *   payload: {data: "value"},
   *   source: {extension: "other-extension", session_id: "other-session", window_id: "target-window"}
   * })
   */
  emit(topic, options = {}) {
    ...
    if (source) {
      ...
      finalSource = {
        extension: source.extension || qiConnection.extension,
      ...
      finalSource = {
        extension: qiConnection.extension,
...
export const initQiConnection = async () => {
  ...
  // Initialize extension (this can be shared across windows)
  let extension = sessionStorage.getItem("qiExtension");
  if (!extension) {
    extension = location.pathname.split("/").filter(Boolean)[0] ?? "";
    sessionStorage.setItem("qiExtension", extension);
  }
  qiConnection.extension = extension;
