...
- Bundle: a collection of addons and environments that specify a reproducible collection.
  A Bundle can have four states:
...
- Addon: a python package containing addon logic, ui logic and plugins in predefined folders.
  An addon is structured this way:
  ...
  Addons have a lifecycle which enable us to auto run different code, namely discover, register, install.
  Addons can also feature settings.

- Plugin: a specialized python class deriving from PluginBase that encapsulates logic to be run in process.
  Plugins can be of different types, each one providing different features.
  A plugin has a register, a discover and a process phase.
  There's a specialized type of plugin - Publish - which inherits not only PluginBase but also pyblish
  instance and context plugin classes. Those plugins will be handled by pyblish on publish.

- Hub: a message hub/pubsub service that dispatches websocket payloads to registered connections.
