"""
This module contains the message bus for the Qi system.
"""
...
from core_new.messaging.connections import ConnectionManager
from core_new.messaging.handlers import HUB_ID, HandlerRegistry
from core_new.models import QiContext, QiMessage, QiMessageType, QiSession
...
class MessageBus:
    ...
    Routes QiMessage between connected sessions over WebSocket.
        ...
        - Maintains a ConnectionManager to track live sockets/sessions
        - Maintains a HandlerRegistry to track topic → handler functions per session
    ...
    def __init__(self) -> None:
        ...
        self._handler_registry: HandlerRegistry = HandlerRegistry()

        self._reply_timeout = app_config.reply_timeout
        self._max_pending = app_config.max_pending_requests_per_session
        ...
        # Track registration tasks to prevent warnings
        self._registration_tasks: Set[asyncio.Task] = set()
        ...
        def decorator(function: Any) -> Any:
            ...
            task = asyncio.create_task(
                self._handler_registry.register(
                    handler_function=function, topic=topic, session_id=session_id
                )
            )
            # Track the task and clean it up when done
            self._registration_tasks.add(task)
            task.add_done_callback(self._registration_tasks.discard)
            return function
