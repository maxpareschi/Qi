...
from core_new.messaging.handlers import HUB_ID
...
class Hub:
    ...
    Under the hood, Hub forwards calls to a MessageBus instance.
    This is where addons and other modules interact with the message bus.
    ...
    def __getattr__(self, name: str) -> Any:
        """
        For any attribute not explicitly defined on Hub, delegate to MessageBus.
        """
        if name.startswith("_"):
            raise AttributeError(f"'Hub' object has no attribute '{name}'")
